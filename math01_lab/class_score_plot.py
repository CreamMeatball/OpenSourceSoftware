...
if __name__ == '__main__':
    ...
    plt.scatter(midtm_kr, final_kr, label='Korean')
    plt.scatter(midtm_en, final_en, label='English')
    plt.legend()

    # Plot total scores as a histogram
    plt.figure()
    plt.hist(total_kr, range=(0,100), bins=20, color='r')
    plt.hist(total_en, range=(0,100), bins=20, color='b', alpha=0.5)
